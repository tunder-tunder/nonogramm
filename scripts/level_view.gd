...
func _update_cell_visual(x: int, y: int) -> void:
	...
	if not button:
		return
	button.text = ""
	var cross_indicator := button.get_meta("cross_indicator", null) as Label
	if not cross_indicator:
		# Supports cells created before a script hot reload and prevents a stale
		# scene instance from dereferencing a missing indicator.
		cross_indicator = _create_cross_indicator()
		button.add_child(cross_indicator)
		button.set_meta("cross_indicator", cross_indicator)
